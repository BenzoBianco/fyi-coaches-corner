...
import { whoIsCalling, resolveRole } from './_lib.js';
...
export default async (req, context) => {
  ...
  /* Coach is always set from who is logged in, never trusted from the
     ...
     Admin staff have no coach identity of their own, so for them body.coach
     must name one, same as it must for Ben. */
  fields['Coach'] = principal ? (body.coach || coach || '') : coach;
