...
const FIELD_MAP = {
  ...
  mobileInContacts:  'Mobile In Contacts',
...
export default async (req, context) => {
  ...
  try {
    ...
    /* A coach can only ever push to a client already assigned to them.
       Ben and admin staff are exempt, full visibility means full edit
       here, per Ben's own instruction. This is checked against
       Airtable's own current value, not whatever the browser sent, so a
       stale local copy can't be used to reassign a client sideways. */
    if (!principal) {
