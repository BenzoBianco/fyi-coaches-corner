...
export default async () => {
  ...
  lines.push('  ADMIN_EMAILS          ' + (adminEmails ? 'set' : 'not set, no admin staff will have access'));
  lines.push('');
  ...
  if (adminEmails) {
    lines.push('ADMIN_EMAILS contents');
    adminEmails.split(',').map(s => s.trim()).filter(Boolean).forEach(e => lines.push('  ' + e));
    lines.push('');
  }
